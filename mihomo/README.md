...
- DNS 防污染接管（macOS：pf 重定向 + DoH；**Linux · Alpine：iptables 重定向 53→1053**；Windows：netsh DNS 修改）
- 交互式节点选择（↑↓ 切换 Enter 确认）
- 节点延迟测速
...
> `install.js` 暂不支持 Windows，需手动完成以下步骤：

1. 安装 [Node.js v16+](https://nodejs.org)
2. 从 [Mihomo Releases](https://github.com/MetaCubeX/mihomo/releases) 下载  
   `mihomo-windows-amd64-vX.X.X.zip`，解压后将 `mihomo.exe` 放到：  
   `C:\Program Files\mihomo\mihomo.exe`
3. 创建配置目录（PowerShell）：
   ```powershell
   New-Item -ItemType Directory -Force "$env:APPDATA\mihomo\data"
   New-Item -ItemType Directory -Force "$env:APPDATA\mihomo\logs"
   ```
4. 将 `config\config.yaml` 复制到 `%APPDATA%\mihomo\config.yaml`，填入节点信息
5. 将本仓库 `src\mihomo-ctl` 下载到本地；在 PATH 可访问的目录（如 `C:\Windows\System32`）中创建包装脚本 `mihomo-ctl.cmd`：
   ```bat
   @echo off
   node "C:\path\to\mihomo-ctl" %*
   ```
6. 以**管理员身份**运行 PowerShell / CMD，然后执行：`mihomo-ctl start`

> DNS 接管（`dns-on`）和系统代理（`proxy-on`）在 Windows 上均需管理员权限。
...
**DNS 防污染（需管理员/sudo，macOS · Linux · Alpine · Windows）**
...
> **Windows**：netsh 修改活跃网络接口 DNS 为 127.0.0.1，需管理员权限。
...
> macOS 使用 pf；Linux / Alpine 使用 iptables；Windows 使用 netsh（均内置于 mihomo-ctl，无需手动配置）。OpenWrt DNS 接管需自行配置。
